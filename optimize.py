...
solvers["pysmtpipe"] = feasibility.pysmt
solvers["smtpipe"] = feasibility.smtpipe
...
def main():
  ...
  accuracy = sympify(str(options.accuracy), rational=True)
  ...
  if (options.solver in pysmtsolvers):
    ...
    verbprint(0, "Error. If 'smtpipe' or 'pysmtpipe' solvers are selected, then options -p or --pipe-to must be used. Use --help for details.", True)
    exit(1)
  elif (options.solver == "smtpipe" and options.pipe_logic == None):
    verbprint(0, "Error. If 'smtpipe' or 'pysmtpipe' solvers are selected, then a logic (e.g. QF_NRA) must be used. Use --help for details.", True)
    exit(1)
  elif (options.solver == "smtpipe" or options.solver == "pysmtpipe"):
  ...
  utils.stats_update("Objective value", float(objval))
