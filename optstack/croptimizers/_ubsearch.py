...
def optimize(objective, accuracy):
  ...
  if (feas_model == None):
    return None
  start_up = solver.get_obj_float(feas_model, accuracy)
  ...
  (lower, upper, gbounds_model) = get_bounds(obj, start_up, accuracy, feas_model)
  ...
  utils.decrease_indentation()
  if (bisec_model != None):
    objval = solver.get_obj_float(bisec_model, accuracy)
...
def get_obj(model, objective, accuracy):
  return solver.get_obj_float(model, accuracy)
