  ...
  if ("e" in s):
    ...
    if pysmt:
      ...
      if (not (is_int(exp))):
        ...
        exit(1)
      exp = int(exp)
      product = "(* " + str(base) + " * 1" + abs(exp) * "0" + ".0)"
      # product = "(* " + str(base) + (abs(exp) * " (* 10.0") + " 1.0" + abs(exp) * ")" + ")"
      if (exp >= 0):
        return product
      else:
        return "(/ 1.0 " + product + ")"
...
# Nonlinear operators
nonlinear = ["abs", "exp", "sin", "cos" "sqrt", "centropy", "ln", "log10", "log2", "logv"]
...
#    return "(abs " + temp + ")"
  ...
  if (pysmt and (exp.tag in binary)):
    ...
    return output
  output = ""
  if (URIstrip(exp.tag) in nonlinear): output = output + "(" + osil2smt[URIstrip(exp.tag)]
  else: output = output + "(" + osil2smt[URIstrip(exp.tag)]
  for op in exp:
...
def generate_objective_smt():
  smt = "(+ 0.0 "
  for a in attrib_of_con[-1]["SMTaddends"]:
    smt = smt + " " + a
  smt = smt + ")"
  attrib_of_con[-1]["SMT"] = smt
  attrib_of[-1]["smt_name"] = smt
  

def generate_constraint_smt():
  for i in range(0, len(attrib_of_con) - 1):
    smt = "(+ 0.0 "
    for a in attrib_of_con[i]["SMTaddends"]:
      smt = smt + " " + a
    smt = smt + ")"
    attrib_of_con[i]["SMT"] = smt
...
def parse_data(instance_data):
  ...
  generate_objective_smt()
  generate_constraint_smt()
...
def make_smt(binflatten):
  ...
  for i in range(0, len(attrib_of_con) - 1):
    ...
    if (i == -1):
      ...
      smt = smt + "(assert (= " + attrib_of_con[i]["SMT"] + " " + lb + "))" + "\n"
      smt = smt + "\n"
    else:
      if (lb != None):
	smt = smt + "(assert (>= " + attrib_of_con[i]["SMT"] + " " + lb + "))" + "\n"
      if (ub != None):
	smt = smt + "(assert (<= " + attrib_of_con[i]["SMT"] + " " + ub + "))" + "\n"
      smt = smt + "\n"
  # if (attrib_of[-1]["maxOrMin"] == "max"):
  #  attrib_of_con[-1]["SMT"] = "(- 0 " + attrib_of_con[-1]["SMT"] + ")"
  # Representation of the objective
#   smt = smt + "(assert (>= " + attrib_of_con[-1]["SMT"] + " " + smt_val(-1e+10) + "))" + "\n"
#   smt = smt + "\n"
#   smt = smt + "(assert (<= " + attrib_of_con[-1]["SMT"] + " " + smt_val(1e+10) + "))" + "\n"
#   smt = smt + "\n"
  smt = smt + "; obj_smt = " + attrib_of_con[-1]["SMT"]
